    ...
    _ajax(null,
        ...
        (data) => {
            ...
            for (var i = 0; i < ListPeriodo.length; i++) {
                ...
                template += '<tr data-id = "' + ListPeriodo[i].fpa_IdFormaPago + '">' +
                    '<td>' + ListPeriodo[i].fpa_Descripcion + '</td>' +
                    '<td>' + ListPeriodo[i].NombreUsuarioCrea + '</td>' +
                    '<td>' + FechaCrea + '</td>' +
                    '<td>' + UsuarioModifica + '</td>' +
                    '<td>' + FechaModifica + '</td>' +
                    '<td>' +
                    '<button data-id = "' + ListPeriodo[i].fpa_IdFormaPago + '" type="button" class="btn btn-primary btn-xs" id="btnEditarPeriodo">Editar</button>' +
        ...
        .done(function (data) {
        	...
        	if (data) {
        		...
        		$.each(data, function (i, iter) {
        			...
        			$("#Editar #peri_Descripcion").val(iter.peri_Descripcion);
        		});
        		$("#EditarPeriodo").modal();
        	}
        	else {
...
$("#btnInactivarPeriodoConfirmar").click(function () {
    ...
    $.ajax({
        url: "/Periodo/Inactivar/" + IDInactivar,
